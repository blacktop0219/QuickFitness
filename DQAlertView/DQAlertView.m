...
// Init method
- (id) initWithTitle:(NSString *) title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle image:(UIImage *)image
{
...
- (void) calculateFrame
{
    viewFullRect = CGRectMake(85, 97, 100, 100);
    if ( ! hasModifiedFrame )
    {
        UIFont * messageFont = self.titleLabel.font ? self.titleLabel.font : [UIFont fontWithName:kHelveticaNeueThin size:18];
    ...
    if ( ! self.message ) {
        ...
        messageLabelFrame = CGRectMake(self.messageLeftRightPadding,
                                       titleLabelFrame.origin.y + titleLabelFrame.size.height + self.titleBottomPadding +15,
                                       self.width - self.messageLeftRightPadding * 2,
                                       self.height - self.buttonHeight - titleLabelFrame.size.height - self.titleTopPadding - self.titleBottomPadding +20);
    } else {
        messageLabelFrame = CGRectMake(self.messageLeftRightPadding,
                                       titleLabelFrame.origin.y +  titleLabelFrame.size.height + self.titleBottomPadding +15,
                                       self.width - self.messageLeftRightPadding * 2,
                                       self.height - titleLabelFrame.size.height - self.titleTopPadding - self.titleBottomPadding +20);
    }
...
- (void) setupItems
{
    ...
    self.viewFulll = [[UIImageView alloc]initWithFrame:CGRectZero];
    
    
    // setup View
    self.viewFulll.backgroundColor = [UIColor clearColor];
//    self.viewFulll.image = [UIImage imageNamed:@"A_cell0.png"];
//    [self.viewFulll setImage:@""];
    
    // Setup Title Label
    self.titleLabel.numberOfLines = 0;
    self.titleLabel.font = [UIFont fontWithName:kHelveticaBold size:21];
    ...
    self.messageLabel.font = [UIFont fontWithName:kHelveticaRegular size:20];
    ...
    [self.cancelButton setTitleColor:[UIColor colorWithRed:53.0/255.0 green:53.0/255.0 blue:53.0/255.0 alpha:1.0] forState:UIControlStateNormal];
    self.cancelButton.titleLabel.font = [UIFont fontWithName:kHelveticaBold size:20];
...
- (void) setupViews
{
    ...
    self.viewFulll.frame = viewFullRect;
    ...
    [self addSubview:self.viewFulll];
...
- (void) cancelButtonClicked: (id) sender
{
    ...
    [self dismiss];
    [appDelegate.PopView removeFromSuperview];
    [appDelegate.popImageView removeFromSuperview];
