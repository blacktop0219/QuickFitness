...
#define SLIDER_X_OFF  -(self.frame.size.width/2)
...
{
    CGRect cframe = CGRectMake(frame.origin.x, frame.origin.y, 81.0f, 33.0f);
    self = [super initWithFrame:cframe];
    if (self) {
        // Initialization code
        self.clipsToBounds = YES;
        CGRect cframe = CGRectMake(self.frame.origin.x, self.frame.origin.y, 81.0f, 33.0f);
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    if(self) {
        self.clipsToBounds = YES;
//        CGRect cframe = CGRectMake(self.frame.origin.x, self.frame.origin.y, 81.0f, 33.0f);
        CGRect cframe = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.frame.size.height);
...
- (void)drawRect:(CGRect)rect {
    ...
    NSString *imgName = appDelegate.CurrentSwitchName;
    UIImageView *slider = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imgName]];
