...
-(float)calendarHeight {
    return kVRGCalendarViewTopBarHeight + [self numRows]*(30+2)+1;
}
...
//    labelCurrentMonth.font = [UIFont fontWithName:kHelveticaNeueThin size:kFontSize25];
    [formatter release];
    ...
    CGContextSetFillColorWithColor(context,
                                   [UIColor colorWithRed:67.0/255.0 green:145.0/255.0 blue:141.0/255.0 alpha:1.0].CGColor);
    ...
    //////  Sun, Mon, Tue Text Color ///// Brijesh
    CGContextSetFillColorWithColor(context,
...
//    NSDate *dateNew = [todayDate dateByAddingTimeInterval:(60*60*24*5)];
//    NSDate *dateNew1 = [todayDate dateByAddingTimeInterval:(60*60*24*7)];
//    NSDate *dateNew2 = [todayDate dateByAddingTimeInterval:(60*60*24*10)];
    
    NSMutableArray *arrDBDate = [[NSMutableArray alloc] init];
    
    NSString * str = [NSString stringWithFormat:@"SELECT CreatedDate FROM UserWorkouts where UserId=%d group by CreatedDate",appDelegate.UserId];
    rs = [db executeQuery:str];
    while ([rs next]) {
        NSDateFormatter *formate = [[NSDateFormatter alloc] init];
        [formate setDateFormat:@"yyyy-MM-dd"];
        NSDate *DBDate = [formate dateFromString:[rs stringForColumnIndex:0]];
        
        [arrDBDate addObject:DBDate];
    }
    
    self.markedDates = [NSArray arrayWithArray:arrDBDate];
    
    for (int i = 0; i<[self.markedDates count]; i++) {
        
        NSDate *MarkdedDate = [self.markedDates objectAtIndex:i];
        
        if ([MarkdedDate month] == [currentMonth month] && [MarkdedDate year] == [currentMonth year]) {
            ...
            int targetY = kVRGCalendarViewTopBarHeight + (targetRow * 30)+3;
            //        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + 38;
            
            CGRect rectangle = CGRectMake(targetX,targetY, 31, 31);
            //        CGContextAddRect(context, rectangle);
            
            UIImage *img = [UIImage imageNamed:@"Cal_event_sel.png"];
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        int targetY = kVRGCalendarViewTopBarHeight + targetRow * 30;
        
        // BOOL isCurrentMonth = NO;
        if (i<firstWeekDay) { //// Previous Month Day Color  /// Brijesh
            targetDate = (prevMonthNumDays-firstWeekDay)+(i+1);
            
            CGContextSetFillColorWithColor(context,
                                           [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:0.4].CGColor);
            
        } else if (i>=(firstWeekDay+currentMonthNumDays)) { //// Next Month Day Color  /// Brijesh
            targetDate = (i+1) - (firstWeekDay+currentMonthNumDays);
            
            CGContextSetFillColorWithColor(context,
                                           [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:0.4].CGColor);
        } else { //// Current Month Day Color  /// Brijesh
            // isCurrentMonth = YES;
            ...
            for (int i = 0; i<[self.markedDates count]; i++) {
                ...
                if ([MarkdedDate month] == [currentMonth month] && [MarkdedDate year] == [currentMonth year] && [MarkdedDate day] == targetDate) {
                    isSameDate = true;
                }
            }
            if(isSameDate){
                CGContextSetFillColorWithColor(context,[UIColor colorWithRed:144.0/255.0 green:186.0/255.0 blue:162.0/255.0 alpha:1.0].CGColor);
            }
            else{
...
//            CGContextFillPath(context);
        ...
        [date drawInRect:CGRectMake(targetX+1, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:kHelveticaNeueThin size:kFontSize22] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
...
////        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + 38;
//        
//        CGRect rectangle = CGRectMake(targetX,targetY, 44, 35);
////        CGContextAddRect(context, rectangle);
//        
//        UIImage *img = [UIImage imageNamed:@"Cal_event_sel.png"];
//        [img drawInRect:rectangle];
...
#pragma mark - Init
-(id)init {
    self = [super initWithFrame:CGRectMake(0, 80, kVRGCalendarViewWidth, 0)];
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont fontWithName:kHelveticaNeueThin size:kFontSize25];
        labelCurrentMonth.textColor = [UIColor colorWithRed:106.0/255.0 green:106.0/255.0 blue:106.0/255.0 alpha:1.0];
